                ...
                tabsName.map((element,index)=>{
                    return(
                        <div className={`lg:text-[16px] xl:text-[16px] md:text-[16px] sm:text-[16px] text-[10px]
                         ${currentTab===element?"bg-blue-300 text-richblack-5 font-medium":"text-blue-100"} 
                         rounded-full transition-all duration-200 cursor-pointer
                          hover:text-richblack-5 hover:bg-richblue-400 md:px-7 lg:px-7 xl:px-7 sm:px-7 md:py-2 lg:py-2 xl:py-2 sm:py-2   px-2 py-2`} key={index} onClick={()=>setMyCard(element)}>
                        {element}
                        </div>
                    )
                })
